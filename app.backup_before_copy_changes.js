(function () {
  ...
  function renderHero() {
    ...
    hero.innerHTML = `
      ...
      <div class="kpi-grid">
        ...
        <div class="kpi"><div class="kpi__value">${top10.length}</div><div class="kpi__label">Deep-review highlight papers</div></div>
  ...
  function renderOverviewCharts() {
    renderBarChart(
      "category-chart",
      Object.entries(countBy(inventory, (paper) => phaseLabels[paper.phase] || paper.phase))
  ...
  function renderReuse() {
    const cards = reuseRules.map(([label, pattern]) => {
      const count = top10.filter((paper) =>
        pattern.test([paper.direct_reuse, paper.framework, paper.findings, paper.india_relevance].join(" "))
      ).length;
      return { label, count };
    });
    document.getElementById("reuse-grid").innerHTML = cards
      .map(
        (card) => `
          <div class="reuse-card">
            <div class="eyebrow">${escapeHtml(card.label)}</div>
            <div class="reuse-count">${card.count}/10</div>
            <div class="small-note">Top papers explicitly support this reusable component.</div>
          </div>
        `
      )
      .join("");
  ...
  function renderInsights() {
    const insights = [
      ...
      `Step 1 is now strong enough to transition into Step 2: selecting climate-sensitive outcomes for India using the highlighted method backbone and evidence inventory.`,
    ];
